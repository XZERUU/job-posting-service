            ...
            <div class="px-6 pb-4">
                ...
                <div>
                    ...
                    <div class="flex items-center gap-4 mt-3 pt-3 border-t border-gray-100">
                        ...
                        <div class="w-px h-6 bg-gray-200"></div>
                        <div class="text-center">
                            <div class="text-sm font-semibold text-green-600">{{ $stats['hired'] ?? 0 }}</div>
                            <div class="text-xs text-gray-400">Hired</div>
                        </div>
                        <div class="w-px h-6 bg-gray-200"></div>
                        <div class="text-center">
                ...
                <div class="bg-white border border-gray-200 rounded-xl p-4 hover:shadow-sm transition-shadow duration-200">
                    ...
                    <div class="space-y-1">
                        ...
                        <div class="flex items-center gap-3 py-2 px-2 hover:bg-gray-50 rounded-lg transition-colors group">
                            ...
                            <div class="flex-1 min-w-0">
                                <div class="text-xs font-medium text-gray-900">{{ $app->job_title }}</div>
                                <div class="text-xs text-gray-400">{{ $app->company_name }}</div>
                            </div>
                            <span class="px-2 py-0.5 text-xs font-medium rounded-md flex-shrink-0
                                @if(in_array($app->status, ['approved', 'hired'])) bg-green-100 text-green-800
                                @elseif(in_array($app->status, ['reviewing', 'under_review', 'interview'])) bg-amber-50 text-amber-700
                                @elseif($app->status === 'rejected') bg-red-50 text-red-700
                                @else bg-gray-100 text-gray-700
                                @endif">
                                {{ in_array($app->status, ['approved', 'hired']) ? 'Hired' : \Illuminate\Support\Str::headline($app->status) }}
                            </span>
                        </div>
                        @empty
